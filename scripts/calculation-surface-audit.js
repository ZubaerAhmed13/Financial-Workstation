...
for(let i=0;i<lines.length;i++){
  const s=lines[i];
  const start=s.match(/\bconst\s+([A-Za-z_$][\w$]*)\s*=\s*\(\s*\(\s*\)\s*=>\s*\{/);if(start)ownerStack.push(start[1]);
  const owner=ownerStack.length?ownerStack[ownerStack.length-1]:null;
  let m;const f=/\bfunction\s+([A-Za-z_$][\w$]*)\s*\(/g;while((m=f.exec(s)))add(m[1],i+1,owner,'function');
...
found.sort((a,b)=>a.line-b.line||a.surface.localeCompare(b.surface));
for(const x of found)Object.assign(x,Registry.classify(x.surface,x.name));
const counts={};for(const s of Registry.STATUSES)counts[s]=found.filter(x=>x.status===s).length;
const migration=counts.REQUIRES_MIGRATION||0;
const coreNames=new Set(Registry.cores.map(c=>c.name));const badRoutes=Manifest.routes.filter(r=>!coreNames.has(r.core));
const routeDuplicates=Manifest.routes.map(r=>r.surface).filter((x,i,a)=>a.indexOf(x)!==i);
const classified=found.length-migration;
let md='# Full Calculation-Surface Classification\n\n';
md+='This report is generated mechanically from the canonical UI source with the generated certification runtime removed before scanning. It is a release gate, not a manually curated assertion.\n\n';
...
if(migration){md+='\n## Required migrations\n\n';for(const x of found.filter(x=>x.status==='REQUIRES_MIGRATION'))md+=`- \`${x.surface}\` at canonical-source line ${x.line}\n`;}
...
const result={source:path.relative(root,target),discovered:found.length,classified,counts,REQUIRES_MIGRATION:migration,badRoutes:badRoutes.length,duplicateRoutes:routeDuplicates.length,ALL_LIVE_FINANCIAL_HELPERS_CERTIFIED:migration===0&&badRoutes.length===0&&routeDuplicates.length===0,out:path.relative(root,out)};
console.log(JSON.stringify(result,null,2));
if(migration||badRoutes.length||routeDuplicates.length)process.exit(1);